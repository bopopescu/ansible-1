...
class Runner(object):
    ...
    def __init__(self,
        host_list=C.DEFAULT_HOST_LIST,      # ex: /etc/ansible/hosts, legacy usage              ;主机列表文件
        module_path=None,                   # ex: /usr/share/ansible                            ;模块路径
        module_name=C.DEFAULT_MODULE_NAME,  # ex: copy                                          ;模块名称
        module_args=C.DEFAULT_MODULE_ARGS,  # ex: "src=/tmp/a dest=/tmp/b"                      ;模块参数
        forks=C.DEFAULT_FORKS,              # parallelism level                                 ;并发数量
        timeout=C.DEFAULT_TIMEOUT,          # SSH timeout                                       ;ssh超时时间
        pattern=C.DEFAULT_PATTERN,          # which hosts?  ex: 'all', 'acme.example.org'       ;host_pattern
        remote_user=C.DEFAULT_REMOTE_USER,  # ex: 'username'                                    ;远程用户名
        remote_pass=C.DEFAULT_REMOTE_PASS,  # ex: 'password123' or None if using key            ;远程密码，如果使用密钥则为None
        remote_port=None,                   # if SSH on different ports                         ;默认是22
        private_key_file=C.DEFAULT_PRIVATE_KEY_FILE, # if not using keys/passwords              ;密钥文件
        background=0,                       # async poll every X seconds, else 0 for non-async  ;异步模式超时时间
        basedir=None,                       # directory of playbook, if applicable              ;playbook目录
        setup_cache=None,                   # used to share fact data w/ other tasks            ;缓存fact
        vars_cache=None,                    # used to store variables about hosts               ;缓存主机变量
        transport=C.DEFAULT_TRANSPORT,      # 'ssh', 'paramiko', 'local'                        ;ssh通道类型
        conditional='True',                 # run only if this fact expression evals to true    ;？
        callbacks=None,                     # used for output                                   ;回调类，用于输出结果
        ...
        role_params=None,                   #
        default_vars=None,                  #
        extra_vars=None,                    # extra vars specified with he playbook(s)          ;外部变量
        is_playbook=False,                  # running from playbook or not?                     ;是否是playbook
        inventory=None,                     # reference to Inventory object                     ;inventory对象
        subset=None,                        # subset pattern                                    ;subset过滤主机列表
        ...
        accelerate=False,                   # use accelerated connection                        ;加速模式，1.5后弃用
