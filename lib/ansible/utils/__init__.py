...
def read_vault_file(vault_password_file):
    ...
    """
    ''' 从文件中读取加密密码，如果文件是可执行的，执行脚本并从标准输出中获取密码'''
    if vault_password_file:
        # 获取绝对路径，os.path库里面有很多函数在文件操作的时候都十分有用
        this_path = os.path.realpath(os.path.expanduser(vault_password_file))
        # 判断文件是否是可执行文件
        if is_executable(this_path):
            try:
                # STDERR not captured to make it easier for users to prompt for input in their scripts
                # 如果文件可执行调用subprocess.Popen函数执行脚本，同时捕获到标准输出中
...
def parse_json(raw_data, from_remote=False, from_inventory=False, no_exceptions=False):
    ...
    # 解析raw_data为json格式数据，使用json.loads

    orig_data = raw_data

    # ignore stuff like tcgetattr spewage or other warnings
    data = filter_leading_non_json_lines(raw_data) # 去掉无效行
...
def parse_extra_vars(extra_vars_opts, vault_pass):
    '''
        ...
        判断三种语法方式：
        --extra-vars "@some_file.json" 格式文件作为参数，以YAML文件的方式解析
        --extra-vars '{"pacman":"mrs","ghosts":["inky","pinky","clyde","sue"]}' 列表或字典格式的以YAML变量格式解析
        --extra-vars "hosts=vipers user=starbuck" key/value格式，按照key/value来解析
    '''
    extra_vars = {}
...
def combine_vars(a, b):
    ...
    if C.DEFAULT_HASH_BEHAVIOUR == "merge":
        ...
        # 字段合并的时候，如果出现key重复，则会以后面的为准，既以b为准
